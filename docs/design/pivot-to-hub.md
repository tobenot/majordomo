...
| **Bifrost 虹桥插件**（新） | 新建 | 装进每个 Claude Code 窗口，用 hook 把工作报告推给中枢。是「窗口 → 中枢」的运输管道。详细施工见 `bifrost-hub-v1.md`。 |
...
> 以下两项在 2026-07-02 转向讨论中已定，详见 `bifrost-hub-v1.md`：
> - **远程推送 notifier**：定 **Bark**（三选一里的一个）。
> - **插件报告协议**：定 **command 脚本一个包办**（因 `Stop` 事件不带助手输出文本，必须脚本读 transcript，纯 http 直连出局）。订阅事件见 `bifrost-hub-v1.md` §2.2。
