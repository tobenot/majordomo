...
### 2026-07-08：`$input` 保留变量导致全部 hook 哑火

**症状**：所有 CC 窗口的 hook 事件（SessionStart、Stop、Notification 等）同时哑火。`/reload-plugins` 加载成功（11 hooks），`/ingest` 手动 curl 可达，但实际 hook 触发时无任何事件到达中枢。report.ps1 exit 0，无报错。

**根因**：`d0fd360` 在 `report.ps1` 的 PreToolUse case 中写了：

```powershell
$input = $evt.tool_input
```

`$input` 是 **PowerShell 保留自动变量**（pipeline input enumerator）。一旦脚本任何位置出现对 `$input` 的赋值，PowerShell 在脚本启动时就将全部 stdin 吞入 `$input`，导致顶部的 `[Console]::In.ReadToEnd()` 读到空字符串——**影响所有事件类型，不止 PreToolUse**。

**修复**：`$input` → `$toolInput`（一行改动）。

**教训**：PowerShell 脚本中禁用以下变量名：
| 变量 | 用途 |
|------|------|
...
| `$_` / `$PSItem` | 管道当前对象 |
| `$this` | 类方法中的当前实例 |
| `$MyInvocation` | 当前执行上下文 |

误用不会报错——`$ErrorActionPreference = 'SilentlyContinue'` 会吞掉赋值失败——但副作用是全局性的。
